...
public class Person extends AbstractEntity implements Comparable<Person> {
	...
	// ここからロジック
	// -------------------------------------------------------
	// ある日に、この人が生きているかを判定する
	public boolean isPersonAlive (Date now) {
		if (this.getDayofdeath() == null) {
			return true;
		}
		return ( ! now.after(this.getDayofdeath()));
	}

	// ある日の人物の年齢を計算する
	public int calculatePersonAge (Date date) {
		...
		// person already dead?
		if ( ! this.isPersonAlive(date)) {
			Calendar death = new GregorianCalendar();
			death.setTime(this.getDayofdeath());
