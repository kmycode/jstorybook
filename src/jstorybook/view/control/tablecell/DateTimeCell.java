...
/**
 * TableViewでDateTimeを表示する
 *
 * @author KMY
 */
public class DateTimeCell<E> extends TableCell<E, Calendar> {
	...
	protected void updateItem (Calendar item, boolean empty) {
		...
		if (item != null) {
			Calendar cal = (Calendar) item.clone();
			cal.add(Calendar.MILLISECOND, TimeZone.getDefault().getRawOffset() * -1);
			this.setText(DateTimeManager.getInstance().dateTimeToString(cal));
		}
		else {
			this.setText("");
		}
