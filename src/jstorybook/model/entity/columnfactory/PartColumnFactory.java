 ...
 * @author KMY
 */
public class PartColumnFactory extends ColumnFactory<Part> {

	private static final PartColumnFactory defaultInstance = new PartColumnFactory();

	private PartColumnFactory () {
	}

	public static PartColumnFactory getInstance () {
		return PartColumnFactory.defaultInstance;
	}

	@Override
	public EditorColumnList createColumnList (Part model) {
		if (model == null) {
			model = new Part();
		}
		...
		columnList.entityTypeProperty().set(EntityType.PART);
		EditorColumn column;
		...
		column = new StringColumn(ResourceManager.getMessage("msg.part.name"), "name");
