...
public abstract class Entity implements Serializable {
	...
	private ReadOnlyLongProperty id;
	private ReadOnlyLongProperty transientId;

	public Entity () {
		super();
		this.id = new SimpleLongProperty(-1L);
		this.transientId = new SimpleLongProperty(Entity.transientIdCounter ++);
	}
	...
	public void setId (Long id) {
		((SimpleLongProperty) this.id).set(id);
	}
	...
	public boolean isTransient() {
		return this.id.getValue() == -1L;
	}

	@Deprecated
	public Long getTransientId() {
		return this.transientId.get();
	}

	public ReadOnlyLongProperty transientIdProperty () {
	...
	public int hashCode() {
		int hash = 7;
		if (this.isTransient()) {
			hash = hash * 31 + Entity.propertyHashCode(this.transientId);
		}
		else {
			hash = hash * 31 + Entity.propertyHashCode(this.id);
		}
		return hash;
	}
	...
	public boolean equals(Object obj) {
		...
		if (this.isTransient()) {
			return Entity.equalsProperty(this.transientId, test.transientId);
		}
		else {
			return Entity.equalsProperty(this.id, test.id);
		}
	}

	static int propertyHashCode (ObservableValue o) {
		return o.getValue() != null ? o.getValue().hashCode() : 0;
	}

	static boolean equalsProperty (ObservableValue o1, ObservableValue o2) {
		return Entity.equalsObject(o1.getValue(), o2.getValue());
	}
	...
	public static boolean equalsListNullValue(
			List<? extends Entity> li1,
			List<? extends Entity> li2) {
		...
		for (Entity e : li1) {
			ids1.add(e.idProperty().get());
		}
		List<Long> ids2 = new ArrayList<Long>();
		for (Entity e : li2) {
			ids2.add(e.idProperty().get());
		}
		ids1.removeAll(ids2);
		return ids1.isEmpty();
	}

	public static int getListHashCode (List<? extends Entity> list) {
		int hash = 31;
		for (Entity o : list) {
			hash = hash * 31 + (o != null ? o.idProperty().getValue().hashCode() : 0);
		}
		return hash;
	}
